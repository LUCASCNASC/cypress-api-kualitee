...
describe('API rest - Integration Detail - /integration/detail', () => {
  ...
  ['token_invalido', null, '', 12345, "' OR 1=1 --"].forEach(token => {
    it(`Falha com token inválido (${JSON.stringify(token)})`, () => {
      integrationDetail({ token, id: validId }).then(response => {
        expect([400, 401, 403]).to.include(response.status);
      });
    });
  });
  ...
  [null, '', 'abc', 0, -1, 999999999, {}, [], true, false].forEach(id => {
    it(`Falha com id inválido (${JSON.stringify(id)})`, () => {
      integrationDetail({ token: validToken, id }).then(response => {
        expect([400, 422, 404]).to.include(response.status);
      });
    });
  });
