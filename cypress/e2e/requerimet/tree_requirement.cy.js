...
describe('API - Requirements Tree Requirements - /requirements/tree_requirements', () => {
  ...
  const validRequirementId = 101; // Substitua por um id de requisito válido do seu ambiente

  // Função utilitária para chamada da API
  function treeRequirements(body, options = {}) {
    return cy.request({
      method: 'POST',
      url: `${BASE_URL}/Requirement/tree_requirements`,
      form: true,
      body,
  ...
  // --- POSITIVO ---
  it('Retorna árvore de requisitos com token, project_id e requirement_id válidos', () => {
    treeRequirements({
      token: validToken,
  ...
  ['project_id', 'requirement_id'].forEach(field => {
    it(`Falha sem campo obrigatório: ${field}`, () => {
      ...
      delete body[field];
      treeRequirements(body).then(response => {
