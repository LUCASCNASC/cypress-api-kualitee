...
describe('API rest - Dashboard Bug Open Status - /dashboard/bugopenstatus', () => {
  ...
  [null, '', 'abc', 0, -1, 999999999, {}, [], true, false].forEach(project_id => {
    it(`Falha com project_id inválido (${JSON.stringify(project_id)})`, () => {
      bugOpenStatus({ token: validToken, project_id }).then(response => {
        expect([400, 422, 404]).to.include(response.status);
      });
    });
  });
  ...
  optionalFields.forEach(field => {
    field.invalids.forEach(invalidValue => {
      it(`Falha com campo opcional '${field.key}' inválido (${JSON.stringify(invalidValue)})`, () => {
        bugOpenStatus({ token: validToken, project_id: validProjectId, [field.key]: invalidValue }).then(response => {
          expect([400, 422, 404]).to.include(response.status);
        });
      });
    });
